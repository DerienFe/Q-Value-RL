class Grid:
    ...
    def display_world_all_q_vals(self, q_star):
        actions = ['N', 'S', 'E', 'W']
        print(' '*10, end='')
        for x in range(self.x_range[0], self.x_range[1]+1):
            print('|{:^5s}|'.format(str(x)), end='')
        print('\n', '-'*(7*(self.x_range[1] - self.x_range[0] + 1)+10))

        for y in range(self.y_range[1], self.y_range[0]-1, -1):
            for action in [Grid.NORTH, Grid.SOUTH, Grid.WEST, Grid.EAST]:
                if action == Grid.SOUTH:
                    print('{:>5s}|{:>5s}'.format(str(y), actions[action-1]), end='')
                else:
                    print('{:>5s}|{:>5s}'.format('',actions[action-1]), end='')
                for x in range(self.x_range[0], self.x_range[1]+1):
                    if (x, y) in self.blocked_states:
                        print('|{:^5s}|'.format('na'), end='')
                    elif (x, y) in self.pos_reward_states:
                        print('|{:^5s}|'.format(
                            '{:.1f}'.format(self.pos_reward_vals[(x, y)])), end='')
                    elif (x, y) in self.neg_reward_states:
                        print('|{:^5s}|'.format(
                            '{:.1f}'.format(self.neg_reward_vals[(x, y)])), end='')
                    else:
                        print('|{:^5s}|'.format(
                            '{:.2f}'.format(q_star[(x, y)][action])), end='')
                print()
            print()
